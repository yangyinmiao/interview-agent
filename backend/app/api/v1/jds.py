...
from app.schemas.jd import JDResponse, JDAnalysisResponse
...
async def list_jds(
    ...
    return [
        JDResponse(id=str(r.id), filename=r.filename, parse_status=r.parse_status, created_at=r.created_at)
        for r in jds
    ]
...
async def get_jd(
    ...
    return JDResponse(
        id=str(jd.id), filename=jd.filename, parse_status=jd.parse_status, created_at=jd.created_at
    )
